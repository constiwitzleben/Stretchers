...
latent space. A single keypoint can then be matched against many deformation hypotheses
...
and re-extracting features. This is robust, but it costs a full forward pass per hypothesis and is
...
deformation-conditioned hypotheses per keypoint. Matching is then repeated with each of the 125 set of hypotheses and the highest confidence matches are selected. This repository includes implementations for Dual Softmax and LightGlue.
...
the published Table 1. The one difference is the Stretcher + LightGlue match score, printed as `−`
in the paper; see [Errata](#errata).
...
produced its results disagree. The code is the groundtruth as it is what generated the released
weights and every reported number. It is left unchanged and the discrepancies are recorded
here.
