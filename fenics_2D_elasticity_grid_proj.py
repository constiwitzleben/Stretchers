...
# def track_pixel_displacement(u, img_width, img_height, l_x, l_y):
#     """
#     Compute the deformed pixel locations based on FEniCS displacement field.
...
#     Returns:
#         new_x, new_y: Deformed pixel coordinate grids.
#         x_min, x_max, y_min, y_max: New image bounds.
#     """
#     # Create a grid of pixel coordinates in the original image
#     x_coords = np.linspace(0, l_x, img_width)
#     y_coords = np.linspace(0, l_y, img_height)
...
#     u_disp = np.array([u((x, y)) for x, y in zip(x_flat, y_flat)])

#     # Compute new positions
#     x_deformed = x_flat + u_disp[:, 0]
#     y_deformed = y_flat + u_disp[:, 1]

#     # Compute new bounds
#     x_min, x_max = x_deformed.min(), x_deformed.max()
#     y_min, y_max = y_deformed.min(), y_deformed.max()

#     # Reshape to image dimensions
#     new_x = x_deformed.reshape((img_height, img_width))
#     new_y = y_deformed.reshape((img_height, img_width))

#     return new_x, new_y, x_min, x_max, y_min, y_max
...
n_x, n_y = 10, 10  # Number of elements
...
texture = pv.read_texture("data/medical_deformed/brain.png")



# Apply the texture and visualize
#plotter = pv.Plotter(window_size=[100, 200])
...
window_height = 1200  # for example, choose a height in pixels
...
#plotter.show_axes()
#plotter.show()
plotter.show(screenshot='data/medical_deformed/deformed_brain.png')
